...
abstract class LibraryLoader {
    private static String getLibraryPath(String libraryBaseName) {
        ...
        return String.format("lexer2/libs/%s-%s%s.so", libraryBaseName, osName, osArch);
    }
    ...
    public static <T> T loadLibraryInJAR(Class<T> interfaceClass, String libraryBaseName) {
        try {
            ...
            InputStream libraryAsStream = new FileInputStream(getLibraryPath(libraryBaseName));
...
//            getLibraryPath(libraryBaseName));
            ...
            T ret = (T) Native.loadLibrary(tmp.getAbsolutePath(), interfaceClass);
            tmp.deleteOnExit();
            return ret;
        } catch (IOException e) {
